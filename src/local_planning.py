...
# Initialize ROS Node
try:
    rospy.init_node("local_planning", anonymous=True)
...
def localise_robot():
    ...
    return np.array(
        [trans.transform.translation.x, trans.transform.translation.y, theta]
    )
...
def is_collision_free(trajectory):
    ...
    map_origin = np.array(
        [map_data.info.origin.position.x, map_data.info.origin.position.y]
    )
...
def generate_controls(last_control):
    ...
    controls = np.array([[v, w] for v in linear_range for w in angular_range])
    return controls
...
def main():
    ...
    global_path.append(
        [bfs_path[-1][0], bfs_path[-1][1], np.pi]
    ...
    while not rospy.is_shutdown() and goal_index < len(global_path):
        ...
        while not rospy.is_shutdown():
            ...
            costs, trajectories = evaluate_controls(
                controls, robot_pose, goal_pose, horizon, dt
            )
            ...
            if (
                stuck_timer_start is not None
                and time.time() - stuck_timer_start > stuck_timeout
            ):
